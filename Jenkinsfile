...
pipeline {
    ...
    environment {
        ...
        ECR_REPOSITORY = 'shopeasy-inventory-service'
    ...
    stages {
        ...
        stage('Unit Tests') { steps { sh 'pip install -r requirements.txt pytest && pytest tests/' } }
        stage('Build Once') {
            steps {
                script {
        ...
        stage('SBOM') { steps { generateSBOM imageRef: env.LOCAL_IMAGE } }
        stage('Push') {
            steps {
                script {
        ...
        stage('Deploy Dev') {
            steps {
                deployHelm release: env.SERVICE_NAME, namespace: 'shopeasy-dev',
                    ...
                    imageTag: env.IMAGE_TAG, imageDigest: env.IMAGE_DIGEST, servicePort: 5003
            }
        }
    }
